...
uint32_t mp3StartPositionForContinuousLoop = 0;
...
unsigned long Adafruit_VS1053_FilePlayer::mp3_ID3Jumper(File mp3)
{
  ...
  Serial.println("mp3 start position");
  Serial.println(start);
  mp3StartPositionForContinuousLoop = start;
  return start;
}
...
void Adafruit_VS1053_FilePlayer::feedBuffer_noLock(void)
{
  ...
  while (readyForData())
  {
    ...
    if (bytesread == 0)
    {
      ...
      // currentTrack.close();
      // break;
      Serial.println("started back at hardcoded beginning");
      currentTrack.seek(mp3StartPositionForContinuousLoop);
